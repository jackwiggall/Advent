...
namespace Advent
{
    class Program
    {
        ...
        void Day2()
        {
            ...
            foreach (string line in lines)
            {
                ...
                else
                {
                    ...
                    if (line0 == 'A' && line2 == 'Y' || line0 == 'B' && line2 == 'Z' || line0 == 'C' && line2 == 'X')
                    { //WIN
                        total1 = total1 + 6;
                        continue;
                    }
        ...
        void Day6()
        {
            string[] lines = System.IO.File.ReadAllLines(@"D:\Work\VS\Advent\inputs\demo7.txt");
            ...
            foreach (string line in lines)
            {
                ...
                for (int i = 13; i < line.Length; i++)
                {
                    ...
                    {
                        if (Array.Exists(letters, element => element == line[j]))
                        {
                            Array.Clear(letters, 0, 15);
                            count = 0;
                            skip = true;
                        }
                        else
                        {
                            letters[count] = line[j];
                            count++;
                        }
                    }
                    if (skip)
        ...
        void Day8() {
            ...
            string[] lines = System.IO.File.ReadAllLines(@"D:\Work\VS\Advent\inputs\day8.txt");
            bool[,] visible = new bool[lines[0].Length,lines.Length];
            int total = 0;

            for (int i=0;i<lines.Length;i++) {
                for (int j=0;j<lines[0].Length;j++) {
                    Console.Write(lines[i][j]);
                    if (i==0||j==0||i==lines.Length-1||j==lines[0].Length-1) { //check if edge
                        total++;
                        visible[j,i] = true;
                        continue;
                    }
                    bool success = true;
                    if (lines[j][i]>lines[j][i-1]){ //|| lines[j][i + 1] < lines[j][i] || lines[j - 1][i] < lines[j][i] || lines[j + 1][i] < lines[j][i]) {
                        for (int k = i-1; k > 0; k--) {
